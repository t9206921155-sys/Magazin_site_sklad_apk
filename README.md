...
```bash
./setup.sh            # установка «из коробки»: deps + мастер .env + запуск на :8000
./setup.sh --help     # локально / VPS / обновление / бот / приложения / проверка
```

Полная инструкция: [SETUP-AUTO.md](SETUP-AUTO.md) (VPS одной командой,
настройка Telegram, сборка APK). Старый `./install.sh` работает как раньше.
