...
по готовым инструкциям. Известный долг: `tests-block14` 29/32 падает и на чистом
`main` (сверка снапшота с живой БД) — отдельная задача на диагностику.
...
- Известный долг: tests-block14 29/32 (pre-existing на `main`, см. «Состояние трека»).
