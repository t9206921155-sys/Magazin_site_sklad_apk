...
def main():
    p=argparse.ArgumentParser(); p.add_argument('backup', type=Path); p.add_argument('--target', type=Path, default=Path('data/shop.db')); p.add_argument('--apply', action='store_true'); a=p.parse_args()
    ...
    a.target.parent.mkdir(parents=True, exist_ok=True); tmp=a.target.with_suffix(a.target.suffix+'.restore-tmp'); shutil.copy2(a.backup,tmp); tmp.replace(a.target); print('Restored successfully'); return 0
