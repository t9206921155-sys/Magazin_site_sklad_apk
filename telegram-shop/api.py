...
def create_app(store, providers: dict, bot=None, notify_new_order=None, notify_order_paid=None,
    ...
    async def api_android_releases(request: Request):
        ...
        latest_version = (data["latest_apk"] or data["latest_aab"] or {}).get("version", "1.1.0")
    ...
    async def android_download_page(request: Request):
        ...
        latest_version = (latest_apk or latest_aab or {}).get("version", "1.1.0")
    ...
    async def content_job_approve(jid:int, body:dict, x_wh_token:str=Header(default=""), x_admin_token:str=Header(default="")):
        ...
        if status not in ("approved","rejected"): raise HTTPException(422,"status должен быть approved или rejected")
        now=datetime.datetime.now().isoformat(timespec="seconds")
        with store._conn: cur=store._conn.execute("UPDATE content_jobs SET status=?,error=?,updated_at=? WHERE id=? AND status='review'",(status,str(body.get("comment","")),now,jid))
        if not cur.rowcount: raise HTTPException(409,"Задача не находится на проверке")
        store.wh_log_add(user["name"], "одобрил контент" if status=="approved" else "отклонил контент", f"job {jid}")
    ...
    async def content_job_result(jid:int, body:dict, x_wh_token:str=Header(default=""), x_admin_token:str=Header(default=""), x_content_signature:str=Header(default="")):
        ...
        if job["status"] == target_status and job["result_url"] == url: return {"ok":True,"status":target_status,"id":jid,"idempotent":True}
    ...
    async def campaign_update(cid:int, body:dict, x_wh_token:str=Header(default=""), x_admin_token:str=Header(default="")):
        ...
        utm=body.get("utm")
        if utm is not None:
            if not isinstance(utm,dict) or any(k not in {"source","medium","campaign","term","content"} for k in utm) or any(len(str(v))>200 for v in utm.values()): raise HTTPException(422,"Недопустимые UTM-параметры")
        ...
        now=datetime.datetime.now().isoformat(timespec="seconds")
        with store._conn:
            if utm is None: cur=store._conn.execute("UPDATE campaigns SET status=?,updated_at=? WHERE id=?",(status,now,cid))
            else: cur=store._conn.execute("UPDATE campaigns SET status=?,utm=?,updated_at=? WHERE id=?",(status,json.dumps(utm),now,cid))
    ...
    @app.post("/api/marketing/publications/{pubid}/dry-run")
    async def publication_dry_run(pubid:int, x_wh_token:str=Header(default=""), x_admin_token:str=Header(default="")):
        wh_user_from_headers(x_wh_token,x_admin_token)
        ...
        return {"ok":True,"mode":"dry-run","external_id":f"stub-{pubid}","channel":row["channel"],"campaign":row["campaign_name"],"utm":json.loads(row["utm"] or "{}")}
    ...
    async def campaign_publications(cid:int, status:str="", x_wh_token:str=Header(default=""), x_admin_token:str=Header(default="")):
        ...
        allowed={"draft","approved","published","failed","rejected"}
        if status and status not in allowed: raise HTTPException(422,"Недопустимый статус публикации")
        query="SELECT * FROM campaign_publications WHERE campaign_id=?" + (" AND status=?" if status else "") + " ORDER BY id DESC"
        args=(cid,status) if status else (cid,)
        return [dict(r) for r in store._q(query,args)]
