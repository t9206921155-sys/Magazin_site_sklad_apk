...
- [ ] Открыть `/crm/` с правами сотрудника и администратора.
- [ ] Проверить отсутствие доступа без warehouse token.
- [ ] Создать задачу с описанием, приоритетом, сроком и ответственным.
- [ ] Изменить статус задачи на `in_progress` и `done`.
- [ ] Проверить общий, складской и sales-каналы.
- [ ] Отправить сообщение двумя сотрудниками и дождаться polling обновления.
