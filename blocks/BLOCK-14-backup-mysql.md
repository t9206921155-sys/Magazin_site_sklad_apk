...
## Фикс теста 12.09.2026 — долг «29/32» закрыт

**Симптом (зафиксирован в отчётах блоков 25/27):** `tests-block14.py` давал 29/32
со сбоем сверки «снапшот ↔ живая БД» — воспроизводимо и на чистом `main`.

**Причина:** тест жёстко смотрел в `telegram-shop/data/shop.db`, тогда как сервер
под тестом (и `run-tests.sh`/CI) поднимается с изолированной базой `MAGAZIN_DB=<временный файл>`.
Итог: `store.export_sqlite_backup` снимал снапшот одной базы, а `--expect-live`/`LIVE`
сверял с другим файлом → 3 ложных падения (количества записей, exit-код и текст
`verify_restore`), при этом механика backup/restore была рабочей.

**Исправление:**
- `tests-block14.py`: `resolve_live_db()` — база берётся из `MAGAZIN_DB` (как у сервера),
  без него — `data/shop.db`; путь печатается в отчёте (первым пунктом), отсутствие
  файла — понятная ошибка вместо трейсбека;
- `run-tests.sh`: `tests-block14.py` подключён в порядок регрессии после block09
  (перед block25/27), с тем же `MAGAZIN_DB`, что у сервера, — теперь CI проверяет
  32/32 на каждой сборке;
- документация долга обновлена (`ROADMAP.md`, `IMPLEMENTATION-QUEUE.md`, отчёты 25/27).

**Проверка (12.09.2026):** изолированный прогон (сервер на `MAGAZIN_DB=/tmp/…` при
«грязной» локальной `data/shop.db`) — **32/32**; прогон без `MAGAZIN_DB` — **32/32**;
подмена пути на несуществующий файл — без падения, **32/32**. Тот же сценарий до фикса: 29/32.
